...
export default function ChamadosAnalistasAdmin() {
  ...
  const buscarChamadosPorView = (filtro: string) => {
    setActiveView(filtro);
    const rota = filtro === "todos"
      ? "/api/usuarios/chamados/admin/"
      : `/api/usuarios/chamados/admin/${filtro}/`;
    buscarChamados(rota);
  };

  const handleFiltro = (filtro: string) => {
    if (perfilUsuario === "analista_admin") {
      buscarChamadosPorView(filtro);
    }
  };
  ...
  useEffect(() => {
    if (perfilUsuario === "analista_admin") {
      buscarChamadosPorView(activeView);
    }
