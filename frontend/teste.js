...
export default function ChamadosUsuariosAdmin() {
  ...
  const fetchChamados = async (rota = "/api/usuarios/chamados/admin/usuarios/") => {
    try {
  ...
  useEffect(() => {
    if (activeView === "meus") {
      fetchChamados("/api/usuarios/chamados/admin/usuarios/");
    }
            ...
            {chamados.length === 0 ? (
              ...
              chamados.map((chamado) => (
                <tr key={chamado.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="py-2">
                    <button className="text-gray-700 hover:text-blue-900 transition-colors" title="Visualizar detalhes">
